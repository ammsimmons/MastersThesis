...
# #' @export
# calc_icc.varde_res <- function(res, rater, subject, scores, k, ci = 0.95){ #file) {
#   #do i need file?

#   #this is written for the case of varde() called prior to calc_icc()
#   if (class(res$model) == "lmerMod") {
#     method <- ggdist::mean_qi
#     dat <- res$model@frame #TODO: access S4 object, this works?!
#   } else{
#     #brms branch
#     method <- ggdist::mode_qi
#     #configure khat and q with dat
#     dat <- res$model$data
...
#     unlist() |>
#     unique()
#   dat <- dat[dat[[subject]] %in% keep, ]

#   # # Remove all raters that had no subjects
#   keep <- lapply(nk, function(x) names(x[x > 0])) |>
#     unlist() |>
#     unique()
#   dat <- dat[dat[[rater]] %in% keep, ]
...
#   balanced <- is_balanced(dat, subject,rater)
#   complete <- is_complete(dat, subject,rater)
#   twoway <- is_twoway(dat, subject, rater)


#   # If not specified, set k as the number of unique raters
#   if (is.null(k)) {
#     k <- length(unique(dat[[rater]]))
#   }
